...
function removeVisitPlanner() {
  ...
  document.querySelectorAll('.nav-menu a[href="#planner"]').forEach((link) => link.remove());
  document.querySelectorAll('.nav-menu a').forEach((link) => {
    if (link.textContent.trim() === 'Πλάνο επίσκεψης') link.remove();
  });
}
...
function insertWeatherNavLink() {
  ...
  const directionsLink = navMenu.querySelector('a[href="#directions"]');
...
function createWeatherSection() {
  ...
  section.id = 'weather';
  section.innerHTML = `
    <div class='section-title centered reveal visible'><p class='eyebrow'>Live Καιρός</p><h2>Ο καιρός τώρα στην Πωγωνιανή.</h2><p>Μια ζωντανή εικόνα για θερμοκρασία, άνεμο, υγρασία και σύντομη πρόγνωση.</p></div>
    <div class='weather-shell reveal visible'>
      <section class='weather-widget' aria-label='Τρέχων καιρός Πωγωνιανής'><div class='weather-topline'><div><p class='eyebrow'>Πωγωνιανή / Βοστίνα</p><h3>Τώρα στο χωριό</h3></div><span class='weather-status' id='weatherStatus'>Σύνδεση...</span></div><div class='weather-main'><div class='weather-temp-row'><span class='weather-icon' id='weatherIcon'>☁️</span><strong class='weather-temp' id='weatherTemp'>--°</strong></div><p class='weather-condition' id='weatherCondition'>Φόρτωση καιρού...</p></div><div class='weather-meta'><article><span>Άνεμος</span><strong id='weatherWind'>-- km/h</strong></article><article><span>Υγρασία</span><strong id='weatherHumidity'>--%</strong></article><article><span>Αίσθηση</span><strong id='weatherFeels'>--°C</strong></article></div></section>
      <aside class='weather-forecast-card' aria-label='Πρόγνωση καιρού Πωγωνιανής'><p class='eyebrow'>Πρόγνωση</p><h3>Επόμενες ημέρες</h3><div class='weather-forecast-list' id='weatherForecastList'><p class='weather-error'>Φόρτωση πρόγνωσης...</p></div><p class='weather-credit' id='weatherUpdated'>Δεδομένα καιρού από Open-Meteo.</p></aside>
    </div>`;
...
const weatherCodeMap = {0:['☀️','Καθαρός ουρανός'],1:['🌤️','Κυρίως αίθριος'],2:['⛅','Μερική συννεφιά'],3:['☁️','Συννεφιά'],45:['🌫️','Ομίχλη'],48:['🌫️','Παγωμένη ομίχλη'],51:['🌦️','Ασθενές ψιλόβροχο'],53:['🌦️','Ψιλόβροχο'],55:['🌧️','Έντονο ψιλόβροχο'],61:['🌧️','Ασθενής βροχή'],63:['🌧️','Βροχή'],65:['🌧️','Έντονη βροχή'],71:['🌨️','Ασθενής χιονόπτωση'],73:['🌨️','Χιονόπτωση'],75:['❄️','Έντονη χιονόπτωση'],80:['🌦️','Τοπικές μπόρες'],81:['🌧️','Μπόρες'],82:['⛈️','Ισχυρές μπόρες'],95:['⛈️','Καταιγίδα'],96:['⛈️','Καταιγίδα με χαλάζι'],99:['⛈️','Ισχυρή καταιγίδα με χαλάζι']};
...
async function loadPogonianiWeather(){
  const status=document.getElementById('weatherStatus'),icon=document.getElementById('weatherIcon'),temp=document.getElementById('weatherTemp'),condition=document.getElementById('weatherCondition'),wind=document.getElementById('weatherWind'),humidity=document.getElementById('weatherHumidity'),feels=document.getElementById('weatherFeels'),forecastList=document.getElementById('weatherForecastList'),updated=document.getElementById('weatherUpdated');
  ...
  try{const response=await fetch(url,{cache:'no-store'});if(!response.ok)throw new Error('Weather request failed');const data=await response.json();const current=data.current;const currentDescription=weatherDescription(current.weather_code);icon.textContent=currentDescription[0];temp.textContent=Math.round(current.temperature_2m)+'°';condition.textContent=currentDescription[1];wind.textContent=Math.round(current.wind_speed_10m)+' km/h';humidity.textContent=Math.round(current.relative_humidity_2m)+'%';feels.textContent=Math.round(current.apparent_temperature)+'°C';status.textContent='Live';forecastList.innerHTML=data.daily.time.map((day,index)=>{const dayDescription=weatherDescription(data.daily.weather_code[index]);const min=Math.round(data.daily.temperature_2m_min[index]);const max=Math.round(data.daily.temperature_2m_max[index]);const rain=data.daily.precipitation_probability_max[index]??0;return `<article class='weather-day'><strong>${dayDescription[0]} ${formatGreekDay(day)}</strong><span>${dayDescription[1]}</span><span>${min}° / ${max}° · ${rain}% βροχή</span></article>`;}).join('');const updatedAt=new Intl.DateTimeFormat('el-GR',{timeZone:'Europe/Athens',day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'}).format(new Date(current.time));updated.textContent='Τελευταία ενημέρωση: '+updatedAt+' · Δεδομένα Open-Meteo.';}catch(error){status.textContent='Offline';condition.textContent='Ο καιρός δεν φορτώθηκε προσωρινά.';forecastList.innerHTML=`<p class='weather-error'>Δεν ήταν δυνατή η φόρτωση της πρόγνωσης. Δοκίμασε ξανά αργότερα.</p>`;}}

function loadMuseumSectionScript(){
  if(document.querySelector('script[src="js/museum-section.js"]'))return;
  const script=document.createElement('script');
  script.src='js/museum-section.js';
  script.defer=true;
