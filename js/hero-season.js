...
weatherStyle.textContent = `
  ...
  .weather-forecast-card {
    ...
    background:
      radial-gradient(circle at 18% 10%, rgba(243, 201, 121, 0.22), transparent 30%),
  ...
  .weather-widget::after {
    ...
    top: -70px;
    border-radius: 999px;
    background: rgba(243, 201, 121, 0.14);
    filter: blur(4px);
  }
  ...
  .weather-status {
    ...
    border-radius: 999px;
    color: var(--gold);
    border: 1px solid rgba(243, 201, 121, 0.35);
    ...
    <div class="weather-shell reveal visible">
      <section class="weather-widget" aria-label="Τρέχων καιρός Πωγωνιανής">
        <div class="weather-topline">
          <div>
            <p class="eyebrow">Πωγωνιανή / Βοστίνα</p>
            <h3>Τώρα στο χωριό</h3>
          </div>
          <span class="weather-status" id="weatherStatus">Σύνδεση...</span>
        </div>

        <div class="weather-main">
          <div class="weather-temp-row">
            <span class="weather-icon" id="weatherIcon">☁️</span>
            <strong class="weather-temp" id="weatherTemp">--°</strong>
          </div>
          <p class="weather-condition" id="weatherCondition">Φόρτωση καιρού...</p>
        </div>
      ...
      <aside class="weather-forecast-card" aria-label="Πρόγνωση καιρού Πωγωνιανής">
        <p class="eyebrow">Πρόγνωση</p>
        <h3>Επόμενες ημέρες</h3>
        <div class="weather-forecast-list" id="weatherForecastList">
          <p class="weather-error">Φόρτωση πρόγνωσης...</p>
        </div>
        <p class="weather-credit" id="weatherUpdated">Δεδομένα καιρού από Open-Meteo.</p>
      </aside>
    </div>
  `;
...
function formatGreekDay(dateString) {
...
async function loadPogonianiWeather() {
  ...
  try {
    ...
    const [currentIcon, currentText] = weatherDescription(current.weather_code);

    icon.textContent = currentIcon;
    temp.textContent = `${Math.round(current.temperature_2m)}°`;
    condition.textContent = currentText;
    ...
    forecastList.innerHTML = data.daily.time.map((day, index) => {
      const [dayIcon, dayText] = weatherDescription(data.daily.weather_code[index]);
      ...
      return `
        <article class="weather-day">
          <strong>${dayIcon} ${formatGreekDay(day)}</strong>
          <span>${dayText}</span>
          <span>${min}° / ${max}° · ${rain}% βροχή</span>
        </article>
      `;
    }).join("");

    const updatedAt = new Intl.DateTimeFormat("el-GR", {
